...
class VacancyRequisitionController extends Controller
{
    ...
    public function create()
    {
        ...
        // Get budgets and budget lines for financial control
        $budgetLines = $this->budgetLinesForCurrentYear();
    ...
    /**
     * Current-year budget lines for vacancy financial control.
     * Loandev budgets have no status column (unlike EYP).
     */
    private function budgetLinesForCurrentYear()
    {
        $budgetQuery = \App\Models\Budget::where('company_id', current_company_id())
            ->where('year', date('Y'));

        if (\Illuminate\Support\Facades\Schema::hasColumn('budgets', 'status')) {
            $budgetQuery->where('status', 'approved');
        }
        ...
        return \App\Models\BudgetLine::where('budget_id', $budget->id)
            ->with('account')
            ->get();
    }
}
