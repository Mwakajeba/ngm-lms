...
class EmployeeController extends Controller
{
    ...
    public function store(Request $request)
    {
        ...
        try {
                ...
                function ($attribute, $value, $fail) use ($currentUser, $request) {
                    if (!empty($value)) {
                        ...
                        if ($existsInEmployees) {
                            $fail('This email address is already registered for another employee.');
                            return;
                        }
                        if ($request->boolean('create_user_account')) {
                            $existsInUsers = User::where('company_id', $currentUser->company_id)
                                ...
                                ->exists();
                            if ($existsInUsers) {
                                $fail('This email address is already registered as a system user.');
                            }
                        }
                    }
                }
            ...
            'phone_number' => [
                ...
                function ($attribute, $value, $fail) use ($currentUser, $request) {
                    ...
                    $existsInEmployees = Employee::where('company_id', $currentUser->company_id)
                        ->where(function ($q) use ($value, $formattedPhone) {
                            $q->where('phone_number', $value)
                              ->orWhere('phone_number', $formattedPhone);
                        })
                        ->exists();
                    if ($existsInEmployees) {
                        $fail('This phone number is already registered for another employee.');
                        return;
                    }
                    // Only enforce uniqueness on users when creating a login account
                    if ($request->boolean('create_user_account')) {
                        $exists = User::where('company_id', $currentUser->company_id)
                            ->where('phone', $formattedPhone)
                            ->exists();
                        if ($exists) {
                            $fail('This phone number is already registered as a system user.');
                        }
                    }
                }
            ],
            ...
            'employment_type' => ['required', 'in:full_time,part_time,contract,probation,casual,intern'],
        ...
        // Empty optional strings → null (DB columns / unique email)
        foreach (['email', 'tin', 'bank_name', 'bank_account_number', 'designation', 'middle_name'] as $optionalField) {
            if (array_key_exists($optionalField, $validated) && ($validated[$optionalField] === '' || $validated[$optionalField] === null)) {
                $validated[$optionalField] = null;
            }
        }
        ...
        if ($createUserAccount) {
            // loandev users.branch_id is NOT NULL — must set on create
            $branchId = $validated['branch_id'] ?? current_branch_id() ?? session('branch_id') ?? $currentUser->branch_id;

            if (empty($branchId)) {
                return redirect()->back()
                    ->withInput()
                    ->withErrors(['branch_id' => 'Branch is required when creating a login user account.']);
            }
            ...
            if (method_exists($employeeUser, 'branches')) {
                $employeeUser->branches()->syncWithoutDetaching([$branchId]);
            }
        ...
        // Create employee record (user_id only when login account was created)
        $employee = Employee::create(array_merge($validated, [
            'company_id' => $currentUser->company_id,
            'user_id' => $employeeUserId,
        ]));

        $successMessage = $createUserAccount
            ? 'Employee created successfully with login user account.'
            : 'Employee created successfully (no login user account).';

        return redirect()->route('hr.employees.index')->with('success', $successMessage);
