        ...
        <table>
            ...
            <tbody>
                <tr>
                    ...
                    <td class="text-end number">{{ number_format($data['assetsTotal'], 2) }}</td>
                    @if($comparativeAsOf && !empty($data['comparativesData']))
                        @php $comp = $data['comparativesData'][0] ?? null; @endphp
                        @if($comp)
                            <td class="text-end number">{{ number_format($comp['assetsTotal'], 2) }}</td>
                            <td class="text-end number {{ ($data['assetsTotal'] - $comp['assetsTotal']) >= 0 ? 'positive' : 'negative' }}">
                                {{ number_format($data['assetsTotal'] - $comp['assetsTotal'], 2) }}
                            </td>
                        @endif
                    @endif
                </tr>
                <tr>
                    ...
                    <td class="text-end number">{{ number_format($data['liabilitiesTotal'], 2) }}</td>
                    @if($comparativeAsOf && !empty($data['comparativesData']))
                        @php $comp = $data['comparativesData'][0] ?? null; @endphp
                        @if($comp)
                            <td class="text-end number">{{ number_format($comp['liabilitiesTotal'], 2) }}</td>
                            <td class="text-end number {{ ($data['liabilitiesTotal'] - $comp['liabilitiesTotal']) >= 0 ? 'positive' : 'negative' }}">
                                {{ number_format($data['liabilitiesTotal'] - $comp['liabilitiesTotal'], 2) }}
                            </td>
                        @endif
                    @endif
                </tr>
                <tr>
                    ...
                    <td class="text-end number">{{ number_format($data['equityTotal'], 2) }}</td>
                    @if($comparativeAsOf && !empty($data['comparativesData']))
                        @php $comp = $data['comparativesData'][0] ?? null; @endphp
                        @if($comp)
                            <td class="text-end number">{{ number_format($comp['equityTotal'], 2) }}</td>
                            <td class="text-end number {{ ($data['equityTotal'] - $comp['equityTotal']) >= 0 ? 'positive' : 'negative' }}">
                                {{ number_format($data['equityTotal'] - $comp['equityTotal'], 2) }}
                            </td>
                        @endif
                    @endif
                </tr>
                ...
                <tr class="check-row">
                    ...
                    @if($comparativeAsOf && !empty($data['comparativesData']))
                        ...
                        @if($comp)
                            ...
                            <td class="text-end number {{ ($data['assetsTotal'] - $comp['assetsTotal']) >= 0 ? 'positive' : 'negative' }}">
                                <strong>{{ number_format($data['assetsTotal'] -
                                 $comp['assetsTotal'], 2) }}</strong>
                            </td>
                        @endif
                    @endif
                </tr>
